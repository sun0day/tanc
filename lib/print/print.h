...
#define tc_print_set_buf(size) _tc_print_set_buf(size, _tc_print_alloc, _tc_print_free)

#define tc_print(stream, ...) \
  do { \
    int writen_size = snprintf(_tc_stdio_buf + _tc_stdio_len, _tc_stdio_caps - _tc_stdio_len, __VA_ARGS__); \
